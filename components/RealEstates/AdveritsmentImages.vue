...
function submitForm() {
    ...
    for (let index = 0; index < files.value.length; index++) {
        const file = {
            ...
            advertisementId: advertismentStore.ReturnNewAdvertismentID
        }
        // Call Api 
...
<template>
    <v-card border>
        <v-form @submit.prevent="submitForm">
            <v-file-input label="رفع الصور" variant="filled" accept="image/jpeg, image/png,image/gif, image/svg,image/bmp, image/webp" border prepend-icon="mdi-camera"
            ...
            <v-row class="mt-2">
                <v-col v-for="(image, i) in imageUrls" :key="i" cols="6" sm="4" md="2">
                    ...
                    <br />
                    <v-text-field active variant="outlined" color="primary"  label="اسم الصورة" v-model="files[i].fileDescription" @change="changeName($event, i)"/>
                </v-col>

            </v-row>
            <v-card-subtitle v-if="files.length > 0" class="mلا-3 pr-0 text-red">*اسم الصورة سيتم عرضه في حالة لم تعرض الصورة
            ...
            <v-btn color="primary" type="submit" size="x-large" class="float-left mt-10"
                :disabled="files.length === 0" :loading="isLaoding" @submit.prevent="submitForm">حفظ </v-btn>
        </v-form>
